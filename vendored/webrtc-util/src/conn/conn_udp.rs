...
impl Conn for UdpSocket {
    ...
    #[cfg(target_os = "linux")]
    async fn send_with_options(&self, buf: &[u8], options: &UdpSendOptions) -> Result<usize> {
        log::debug!(
            "UdpSocket::send_with_options called with TTL={:?}, TOS={:?}, DF={:?}",
            options.ttl,
            options.tos,
            options.df_bit
        );
        ...
        if let Some(remote_addr) = self.peer_addr().ok() {
            log::debug!(
                "UdpSocket: Forwarding to send_to_with_options_impl for addr={}",
                remote_addr
            );
    ...
    async fn send_to_with_options(
        ...
        log::debug!(
            "UdpSocket::send_to_with_options called with TTL={:?}, TOS={:?}, DF={:?}, target={}",
            options.ttl,
            options.tos,
            options.df_bit,
            target
...
async fn send_to_with_options_impl(
    ...
    log::debug!(
        "send_to_with_options_impl: buf_len={}, TTL={:?}, target={}",
        buf.len(),
        options.ttl,
        target
    );
    ...
    let result = task::spawn_blocking(move || sendmsg_with_options(fd, &buf, target, &options))
        .await
        .map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e.to_string()))??;

    log::debug!(
        "send_to_with_options_impl: Successfully sent {} bytes",
        result
    );
...
fn sendmsg_with_options(
    ...
    log::debug!(
        "sendmsg_with_options: fd={}, buf_len={}, dest={}, TTL={:?}, TOS={:?}, DF={:?}",
        fd,
        buf.len(),
        dest,
        options.ttl,
    ...
    unsafe {
        ...
        log::debug!(
            "sendmsg_with_options: Socket family={}, is_ipv6={}",
            socket_family,
            is_ipv6_socket
        );
        ...
        if is_ipv6_socket {
            ...
            if let Some(ttl) = options.ttl {
                log::debug!(
                    "sendmsg: Adding IPv6 hop limit control message: TTL={}",
                    ttl
                );
            ...
            if let Some(tos) = options.tos {
                log::debug!(
                    "sendmsg: Adding IPv6 traffic class control message: TOS={}",
                    tos
                );
            ...
            if let Some(ttl) = options.ttl {
                ...
                if !cmsg.is_null() {
                    ...
                    *(data_ptr as *mut i32) = ttl as i32;
                    ...
                    cmsg_len = (*cmsg).cmsg_len;
                    log::debug!(
                        "sendmsg: Set IPv4 TTL={} in control message, cmsg_len={}",
                        ttl,
                        cmsg_len
                    );
                }
        ...
        if result < 0 {
            ...
            log::error!(
                "❌ sendmsg FAILED with error: {} (errno={})",
                err,
                ...
                SocketAddr::V4(addr_v4) => {
                    ...
                    let (src_ip, src_port) = match local_addr {
                        Ok(SocketAddr::V4(src)) => {
                            (u32::from_be_bytes(src.ip().octets()), src.port())
                        }
...
mod tests {
    ...
    async fn test_ipv4_socket_family() {
        ...
        assert_eq!(
            ...
            "IPv4 socket should have AF_INET family"
        );
    ...
    async fn test_ipv6_socket_family() {
        ...
        assert_eq!(
            ...
            "IPv6 socket should have AF_INET6 family"
        );
