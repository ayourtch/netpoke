...
pub type TrackingCallback = Box<
    dyn Fn(SocketAddr, Option<SocketAddr>, u16, Option<u8>, Vec<u8>, Instant, String, u16)
        + Send
        + Sync,
>;
...
where
    F: Fn(SocketAddr, Option<SocketAddr>, u16, Option<u8>, Vec<u8>, Instant, String, u16)
        + Send
        + Sync
        + 'static,
{
...
pub extern "C" fn netpoke_track_udp_packet(
    ...
    let cleartext = unsafe { std::slice::from_raw_parts(buf_ptr, buf_len).to_vec() };
    ...
    let dest_addr = SocketAddr::from((std::net::Ipv4Addr::from(dest_ip_v4), dest_port));

    let src_addr = SocketAddr::from((std::net::Ipv4Addr::from(src_ip_v4), src_port));
...
pub extern "C" fn netpoke_track_udp_packet_v6(
    ...
    let dest_addr = SocketAddr::from((std::net::Ipv6Addr::from(dest_ip_bytes), dest_port));

    let src_addr = SocketAddr::from((std::net::Ipv6Addr::from(src_ip_bytes), src_port));
    let src_addr = Some(src_addr);
...
mod tests {
    ...
    fn test_udp_checksum_v4_different_payload() {
        ...
        let checksum1 =
            calculate_udp_checksum_v4(src_ip, dest_ip, src_port, dest_port, b"Payload A");
        let checksum2 =
            calculate_udp_checksum_v4(src_ip, dest_ip, src_port, dest_port, b"Payload B");
    ...
    fn test_udp_checksum_v4_different_ports() {
        ...
        assert_ne!(
            checksum1, checksum2,
            "Different src ports should produce different checksums"
        );
        assert_ne!(
            checksum1, checksum3,
            "Different dest ports should produce different checksums"
        );
    }
    ...
    fn test_udp_checksum_v6_different_addresses() {
        ...
        assert_ne!(
            checksum1, checksum2,
            "Different src addresses should produce different checksums"
        );
    }
    ...
    fn test_udp_checksum_empty_payload() {
        ...
        assert_ne!(
            checksum, 0,
            "Empty payload should produce non-zero checksum"
        );
    }
}
