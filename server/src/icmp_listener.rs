use crate::packet_tracker::{
    EmbeddedUdpInfo, IcmpMessageClass, PacketTracker, MAX_PAYLOAD_PREFIX_SIZE,
};
...
fn parse_icmp_error(packet: &[u8]) -> Option<(EmbeddedUdpInfo, IcmpMessageClass)> {
    ...
    let src_port = u16::from_be_bytes([packet[embedded_udp_start], packet[embedded_udp_start + 1]]);
