...
impl GitHubProvider {
    pub fn new(config: &OAuthConfig) -> Result<Self, AuthError> {
        ...
        let client_secret = config.github_client_secret.as_ref()
            .ok_or_else(|| AuthError::ConfigError("GitHub client_secret not configured".to_string()))?
            .clone();
        let redirect_url = config.github_redirect_url.as_ref()
            .ok_or_else(|| AuthError::ConfigError("GitHub redirect_url not configured".to_string()))?
            .clone();
