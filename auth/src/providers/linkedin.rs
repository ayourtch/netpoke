...
use oauth2::{basic::BasicClient, AuthUrl, ClientId, CsrfToken, RedirectUrl, Scope, TokenUrl};
...
impl LinkedInProvider {
    pub fn new(config: &OAuthConfig) -> Result<Self, AuthError> {
        ...
        let client_secret = config
            ...
            .as_ref()
            .ok_or_else(|| {
                AuthError::ConfigError("LinkedIn client_secret not configured".to_string())
            })?
            .clone();
        let redirect_url = config
            ...
            .as_ref()
            .ok_or_else(|| {
                AuthError::ConfigError("LinkedIn redirect_url not configured".to_string())
            })?
            .clone();
