...
use crate::session::{AuthProvider, OAuthEndpoints, OAuthTempState, SessionData};
...
impl BlueskyProvider {
    pub fn new(config: &OAuthConfig, dns_resolver: TokioAsyncResolver) -> Result<Self, AuthError> {
        ...
        let redirect_url = config
            ...
            .as_ref()
            .ok_or_else(|| {
                AuthError::ConfigError("Bluesky redirect_url not configured".to_string())
            })?
            .clone();
    ...
    pub async fn start_auth(&self, handle: &str) -> Result<(String, OAuthTempState), AuthError> {
        ...
        let (auth_url, _) = oauth_client
            ...
            .add_scope(Scope::new(
                "rpc:app.bsky.actor.getProfile?aud=did:web:api.bsky.app#bsky_appview".to_string(),
            ))
    ...
    pub async fn complete_auth(
        ...
        let dpop_key = temp_state
            .dpop_private_key
            .as_ref()
            .ok_or_else(|| AuthError::InvalidSession)?;
        let oauth_endpoints = temp_state
            .oauth_endpoints
            .as_ref()
            .ok_or_else(|| AuthError::InvalidSession)?;
        let user_did = temp_state
            .user_did
            .as_ref()
            .ok_or_else(|| AuthError::InvalidSession)?;
        let handle = temp_state
            .handle
            .as_ref()
            .ok_or_else(|| AuthError::InvalidSession)?;
        ...
        let dpop_proof = create_dpop_proof(dpop_key, "POST", &oauth_endpoints.token_url, None)?;
    ...
    async fn fetch_profile(
        ...
        let dpop_proof_api =
            create_dpop_proof_with_ath(dpop_key, "GET", &api_url, None, Some(access_token))?;
        ...
        if profile_response.status().is_success() {
            ...
            Err(AuthError::NetworkError(reqwest::Error::from(
                profile_response.error_for_status().unwrap_err(),
            )))
        }
    }
    ...
    async fn discover_oauth_endpoints(
        ...
        let auth_server = self
            .get_protected_resource_metadata(&service_endpoint)
            .await?;
...
fn create_dpop_proof_with_ath(
    ...
    let header_b64 = URL_SAFE_NO_PAD
        .encode(serde_json::to_string(&header).map_err(|e| AuthError::JsonError(e))?);
    let claims_b64 = URL_SAFE_NO_PAD
        .encode(serde_json::to_string(&claims).map_err(|e| AuthError::JsonError(e))?);
